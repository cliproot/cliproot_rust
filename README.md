...
### Background knowledge automation (optional)

When the plugin's `level` user config is set to `digest` or `wiki`, cliproot synthesises background digests and wiki pages via the Anthropic Messages API after each Claude Code session. It needs credentials — resolved in this order:

1. **`ANTHROPIC_API_KEY`** — a raw API key from [console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys). Pay-as-you-go billing.
2. **`CLAUDE_CODE_OAUTH_TOKEN`** — generated by running `claude setup-token` once. **Recommended on macOS**, where the OAuth credentials Claude Code stores in Keychain are not readable from hook subprocesses. Uses your existing Claude Code subscription.
3. **`$CLAUDE_CONFIG_DIR/.credentials.json`** — if `CLAUDE_CONFIG_DIR` is set.
4. **`~/.claude/.credentials.json`** — Linux/Windows default location for Claude Code's stored login.

If you leave the plugin at its default `level: curator`, no Anthropic credentials are ever needed — the knowledge-automation hooks stay idle.

Quick setup on macOS:

```bash
claude setup-token
# prints an sk-ant-oat01-... token and sets CLAUDE_CODE_OAUTH_TOKEN in your shell profile
```

If background flush or compile ever fails (for any reason, including a missing credential), the outcome is appended to `.cliproot/knowledge/log.md` — no session is interrupted.
