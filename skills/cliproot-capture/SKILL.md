...
2. **Check prior work**: Before starting research, call `cliproot_wiki_query` to see if the topic has been covered before — reuse beats rediscovery.
3. **Capture**: When encountering a source worth citing, `cliproot_clip_create` the relevant passage. Set `source_type` appropriately.
3. **Synthesize**: When combining or summarizing clips, `cliproot_clip_derive` with parent clip IDs and accurate `transformation_type`.
4. **Cite**: Use `cliproot_doc_annotate` for inline citations and `cliproot_doc_cite` for bibliography in final output.
...
If no hooks are available (Windsurf), call `cliproot_session_consolidate` periodically to check for unhighlighted sources.
